...
* (1) 자료형의 참과 거짓
    ...
    * 정리 하자면, 문자열, 리스트, 튜플 등이 비어 있으면 거짓(False)이 된다. 그리고 비어 있지 않다면 거짓(False)이 된다.
    
    * 숫자는 값이 0일 때, 거짓(False)이 된다.

* (2) 비어있는 리스트 확인

    * 비어있는 Sequence(String / List / Tuple)는 False 값을 가진다.
    
    * 비어있는 리스트를 확인하려면 다음과 같이 한다.
