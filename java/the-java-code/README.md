...
* JVM 구조
    ...
    * (2) `메모리(Runtime Data Area)`
        ...
        * ③ `스택(Stack) 영역`
            ...
            * 각 쓰레드 마다 런타임 스택을 하나씩 만들고, 그 안에 메소드 호출을 스택 프레임이라 부르는 블럭으로 쌓아 올라갑니다. (지역변수 및 매개변수 포함)
    ...
    * (3) `실행 엔진(Execution Engine)`
        ...
        * ② `JIT 컴파일러(Just-In-Time Compiler)`
            ...
            * 인터프리터가 반복되는 바이트 코드를 발견하면 JIT 컴파일러로 반복되는 코드를 전부 네이티브 코드로 변환한다.
            
            * 그 다음 부터 인터프리터는 네이티브 코드로 변환된 코드를 사용한다. (프로그램 실행 속도를 향상 시킴)
    ...
    * (5) `네이티브 메소드 라이브러리(Native Method Libraries)` : 자바가 아닌 언어(C, C++, 어셈블리)로 작성된 라이브러리다.
...
* 자바 실행 과정
    ...
    * ③ 실행 엔진(Execution Engine)이 바이트 코드를 한 줄씩 읽어서 네이티브 코드로 변환한 다음, 실행한다.
        ...
        * 메모리 또는 실행 엔진이 네이티브 메소드 라이브러리를 사용한다면 JNI를 통해서 사용한다.
